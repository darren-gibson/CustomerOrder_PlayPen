namespace CustomerOrder.Model.UnitTests
{
    ...
    public class EventRasingCustomerOrderFactoryShould
    {
        ...
        {
            _mockFactory.Setup(f => f.MakeCustomerOrder(_expectedIdentifier)).Returns(_expectedOrder);
            var actualOrder = _factoryUnderTest.MakeCustomerOrder(_expectedIdentifier);
        ...
        public void RaisesAnEventContainingTheOrderWhenANewOneIsMade()
        {
            ...
            _factoryUnderTest.MakeCustomerOrder(_expectedIdentifier);
        ...
        public void CallsTheInnerFactoryAndReturnsTheOrderFromThatWhenAnExistingOrderIsMade()
        {
            ...
            _mockFactory.Setup(f => f.MakeCustomerOrder(_expectedIdentifier, expectedEvents, expectedPricedOrder)).Returns(_expectedOrder);
            var actualOrder = _factoryUnderTest.MakeCustomerOrder(_expectedIdentifier, expectedEvents, expectedPricedOrder);
            Assert.AreEqual(_expectedOrder, actualOrder);
        }
        ...
        public void RaisesAnEventContainingTheOrderWhenAnExistingOneIsMade()
        {
            ...
            _factoryUnderTest.CustomerOrderMade += _factoryUnderTest_CustomerOrderMade;
            _factoryUnderTest.MakeCustomerOrder(_expectedIdentifier, expectedEvents, expectedPricedOrder);
            Assert.IsTrue(_customerOrderMadeWasCalled);
        }
        #endregion
